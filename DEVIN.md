...
binary download or production-ready public release. Apple Silicon and Intel
builders can use the one-command source installer to build, ad-hoc sign, and
install a normal app under `~/Applications`; no Apple Developer identity or
Gatekeeper bypass is used. The installed Apple Silicon bundle proves that the
packaged desktop launches its sibling `buzz-acp`, which in turn launches the
official `devin acp`. A same-name development-agent DM was
